...
def require_api_key(
    ...
    if not settings.planner_api_keys:
        ...
        logger.debug(
            "API key authentication skipped - no keys configured", extra={"request_id": request_id}
        )
        return ""
    ...
    if x_api_key is None:
        logger.warning(
            "Authentication failed: Missing X-API-Key header", extra={"request_id": request_id}
        )
        raise HTTPException(
    ...
    if not settings.is_api_key_valid(x_api_key):
        logger.warning("Authentication failed: Invalid API key", extra={"request_id": request_id})
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Invalid API key")

    # Authentication successful
    logger.debug("Authentication successful", extra={"request_id": request_id})
