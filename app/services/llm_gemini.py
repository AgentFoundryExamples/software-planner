...
class GeminiClient(BaseLLMClient):
    ...
    **Important**: This client uses synchronous calls with blocking sleep for retry backoff.
    ...
    def _call_llm_api(self, description: str, system_prompt: str) -> str:
        ...
        log_llm_request(
            logger=logger, provider="google", model=self.model, description_length=len(description)
        )
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                if isinstance(e, LLMResponseError):
                    logger.error("Gemini API returned invalid response", extra={"error": str(e)})
                    raise

                # Check for authentication/permission errors (not retryable)
                if any(
                    keyword in error_str
                    for keyword in ["auth", "permission", "forbidden", "401", "403"]
                ):
                    logger.error(
                ...
                if "404" in error_str or "not found" in error_str:
                    logger.error(
                        "Gemini resource not found", extra={"error": str(e), "model": self.model}
                    )
                    raise LLMConfigurationError(
                ...
                if retry_count >= self.max_retries:
                    ...
                    metrics = get_metrics_collector()
                    metrics.record_llm_request(
                        provider="google", model=self.model, status="error", duration=elapsed
                    )
