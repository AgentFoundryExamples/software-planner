...
class JobRepository:
    ...
    async def create_job(
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                logger.info("Job created successfully", extra={"job_id": job_id, "model": model})
                ...
                log_job_transition(
                    logger=logger, job_id=job_id, from_status=None, to_status="QUEUED", model=model
                )
                ...
                if not is_duplicate:
                    ...
                    if (
                        "duplicate key" in error_str or "unique constraint" in error_str
                    ) and "job_id" in error_str:
                        is_duplicate = True
        ...
        raise JobRepositoryError(f"Failed to generate unique job_id after {max_retries} attempts")
        ...
        try:
            async with self.engine.begin() as conn:
                ...
                await conn.execute(
                    ...
                    {"job_id": job_id, "status": "RUNNING", "started_at": now, "updated_at": now},
                )
            ...
            logger.info("Job marked as RUNNING", extra={"job_id": job_id})
            ...
            # Log structured transition
            log_job_transition(
                logger=logger, job_id=job_id, from_status="QUEUED", to_status="RUNNING"
            )
    ...
    async def mark_succeeded(self, job_id: str, result: dict) -> Job:
        ...
        try:
            ...
            logger.info("Job marked as SUCCEEDED", extra={"job_id": job_id})
            ...
            # Log structured transition
            log_job_transition(
                logger=logger, job_id=job_id, from_status="RUNNING", to_status="SUCCEEDED"
            )
    ...
    async def mark_failed(self, job_id: str, error: dict) -> Job:
        ...
        try:
            ...
            logger.info(
                "Job marked as FAILED", extra={"job_id": job_id, "error_type": error.get("type")}
            )
