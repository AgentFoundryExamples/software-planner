...
@pytest.mark.skip(
    reason="Requires complex database mocking - sync endpoint tests provide sufficient coverage"
)
    ...
    def client_with_async_mocks(self, mock_llm_client, mock_rate_limiter):
        ...
        with patch("app.services.store_singleton.get_llm_client", return_value=mock_llm_client):
            with patch("app.services.store_singleton.get_job_store", return_value=mock_job_repo):
                with patch(
                    "app.services.rate_limiter.get_rate_limiter", return_value=mock_rate_limiter
                ):
                    with patch("app.api.routes.get_rate_limiter", return_value=mock_rate_limiter):
...
class TestRateLimitingWithRealLimiter:
    ...
    def client_with_real_limiter(self, mock_llm_client):
        ...
        real_limiter = RateLimiter(window_seconds=60, max_requests=3, enabled=True)
            ...
            with patch("app.api.routes.get_rate_limiter", return_value=real_limiter):
                with patch("app.api.routes.get_job_store", return_value=mock_job_repo):
                    ...
                    with patch("app.api.dependencies.settings") as mock_settings:
                        ...
                        mock_settings.planner_api_keys = [
                            "dummy-key"
                        ]  # At least one key configured
                        mock_settings.is_api_key_valid.return_value = (
                            True  # All keys valid for testing
                        )
    ...
    def test_different_keys_have_separate_limits(self, client_with_real_limiter):
        ...
        for i in range(3):
            response = client.post(
                "/api/v1/plan", json={"description": f"Build API #{i}"}, headers={"X-API-Key": key1}
            )
            assert (
                response.status_code == 200
            ), f"Key1 request {i} failed with status {response.status_code}"

        # Key 1's 4th request is denied
        response = client.post(
            "/api/v1/plan", json={"description": "Build API #4"}, headers={"X-API-Key": key1}
        )
        assert (
            response.status_code == 429
        ), f"Key1 should be rate limited but got {response.status_code}"

        # Key 2 can still make requests (fresh bucket)
        response = client.post(
            "/api/v1/plan", json={"description": "Build API"}, headers={"X-API-Key": key2}
        )
    ...
    def test_ip_based_rate_limiting_for_anonymous(self, client_with_real_limiter):
        ...
        # First 3 requests should succeed
        for i in range(3):
            response = client.post("/api/v1/plan", json={"description": f"Build API #{i}"})
...
class TestRateLimitingDisabled:
    ...
    def test_disabled_rate_limiter_allows_all_requests(self, mock_llm_client):
        ...
        disabled_limiter = RateLimiter(
            window_seconds=60, max_requests=1, enabled=False  # Very low limit, but disabled
        )
        ...
        with patch("app.services.store_singleton.get_llm_client", return_value=mock_llm_client):
            with patch("app.api.routes.get_rate_limiter", return_value=limiter):
                with patch("app.api.routes.get_job_store", return_value=mock_job_repo):
                    ...
                    response = client.post("/api/v1/plan", json={"description": "Build API"})
        ...
        with patch("app.services.store_singleton.get_llm_client", return_value=mock_llm_client):
            with patch("app.api.routes.get_rate_limiter", return_value=limiter):
                with patch("app.api.routes.get_job_store", return_value=mock_job_repo):
                    ...
                    response = client.post(
                        "/api/v1/plan",
                        json={"description": "Build API", "system_prompt": "Custom prompt"},
                        headers={"X-API-Key": "test-key"},
                    )
