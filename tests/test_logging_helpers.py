...
class TestCreateStructuredLogContext:
    ...
    def test_extra_fields(self):
        ...
        context = create_structured_log_context(
            request_id="req-123", custom_field="value", another_field=42
        )
    ...
    def test_none_values_excluded(self):
        ...
        context = create_structured_log_context(
            request_id="req-123", job_id=None, custom_field=None
        )
...
class TestLogJobTransition:
    ...
    def test_log_job_created(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            log_job_transition(
                logger=logger, job_id="job-123", from_status=None, to_status="QUEUED"
            )
    ...
    def test_log_job_transition(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            log_job_transition(
                logger=logger, job_id="job-123", from_status="QUEUED", to_status="RUNNING"
            )
...
class TestLogLlmRequest:
    ...
    def test_log_llm_request(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            log_llm_request(
                logger=logger, provider="openai", model="gpt-4", description_length=1000
            )
...
class TestLogHttpRequest:
    ...
    def test_log_http_request(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            log_http_request(
                logger=logger, method="POST", endpoint="/api/v1/plans", status=202, duration=0.5
            )
