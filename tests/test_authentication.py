...
class TestAPIKeyAuthentication:
    ...
    def test_post_plan_with_empty_api_key(self, client_with_api_keys):
        ...
        response = client_with_api_keys.post(
            "/api/v1/plan", json={"description": "Build a REST API"}, headers={"X-API-Key": ""}
        )
...
class TestAPIKeyAuthenticationAsync:
    ...
    def test_post_plans_without_api_key(self, client_with_api_keys):
        ...
        response = client_with_api_keys.post(
            "/api/v1/plans", json={"description": "Build a REST API"}
        )
...
class TestAuthenticationQuotaExceedance:
    ...
    def test_api_key_quota_concept_verified(self):
        ...
        limiter = RateLimiter(
            ...
            max_requests=2,
            enabled=True,
            per_key_overrides={"premium-key": 10, "basic-key": 1},
        )

        # Premium key should get higher quota
        for i in range(10):
            allowed, _ = limiter.check_rate_limit(api_key="premium-key", request_id=f"req-{i}")
            assert allowed is True, f"Premium key should allow request {i+1}"

        # 11th request should be denied
        allowed, retry_after = limiter.check_rate_limit(api_key="premium-key", request_id="req-11")
    ...
    def test_independent_quotas_per_key(self):
        ...
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(api_key="key1", request_id=f"req-key1-{i}")
            assert allowed is True

        # Key1 is rate limited
        allowed, _ = limiter.check_rate_limit(api_key="key1", request_id="req-key1-3")
        assert allowed is False

        # Key2 should still have full quota
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(api_key="key2", request_id=f"req-key2-{i}")
