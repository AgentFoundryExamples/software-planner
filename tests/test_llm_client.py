...
class TestProviderAdapters:
    ...
    @patch('app.services.llm_openai.OpenAI')
    def test_openai_adapter_raises_on_auth_error(self, mock_openai_class):
        """Test that OpenAI adapter raises LLMRequestError on auth failure."""
        from app.services.llm_openai import OpenAIClient
        from openai import AuthenticationError
        
        mock_client_instance = mock_openai_class.return_value
        # Create a proper mock response object with request and status_code attributes
        mock_request = type('obj', (object,), {'headers': {}})()
        ...
        client = OpenAIClient(api_key="invalid-key", model="gpt-4")
    ...
    @patch('app.services.llm_openai.OpenAI')
    def test_openai_adapter_handles_rate_limit(self, mock_openai_class):
        """Test that OpenAI adapter retries on rate limit error."""
        from app.services.llm_openai import OpenAIClient
        ...
        # Create proper mock response object for RateLimitError with status_code
        mock_request = type('obj', (object,), {'headers': {}})()
        mock_response_obj = type('obj', (object,), {
            'request': mock_request,
            'status_code': 429
