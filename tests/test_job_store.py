...
class TestJobStoreEdgeCases:
    ...
    def test_update_job_with_all_fields(self):
        ...
        updated = asyncio.run(
            store.update_job(job.job_id, status="FAILED", result=result, error=error)
        )
...
class TestJobStoreThreadSafety:
    ...
    def test_concurrent_job_creation(self):
        ...
        def create_jobs():
            return [
                asyncio.run(store.create_job(description="Test description"))
...
class TestJobStoreMetadata:
    ...
    def test_create_job_with_system_prompt_metadata(self):
        ...
        store = JobStore()
        job = asyncio.run(
            store.create_job(description="Test description", system_prompt="abc123def456")
        )
    ...
    def test_create_job_with_both_metadata_fields(self):
        ...
        job = asyncio.run(
            store.create_job(
                description="Test description", model="claude-opus", system_prompt="xyz789"
            )
        )
    ...
    def test_get_job_preserves_metadata(self):
        ...
        job = asyncio.run(
            store.create_job(
                description="Test description", model="gpt-4-turbo", system_prompt="hash123"
            )
        )
    ...
    def test_list_jobs_includes_metadata(self):
        ...
        job2 = asyncio.run(
            store.create_job(description="Test description", system_prompt="hash123")
        )
    ...
    def test_metadata_with_failed_job(self):
        ...
        job = asyncio.run(
            store.create_job(
                description="Test description", model="claude-opus", system_prompt="hash456"
            )
        )
