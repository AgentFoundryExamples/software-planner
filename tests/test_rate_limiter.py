...
class TestRateLimiter:
    ...
    def test_rate_limiter_allows_request_within_limit(self):
        ...
        allowed, retry_after = limiter.check_rate_limit(api_key="test-key-123", request_id="req-1")
    ...
    def test_rate_limiter_denies_request_over_limit(self):
        ...
        limiter = RateLimiter(window_seconds=60, max_requests=3, enabled=True)

        # Make 3 requests (should all succeed)
        for i in range(3):
            allowed, _ = limiter.check_rate_limit(api_key="test-key-123", request_id=f"req-{i}")
            assert allowed is True

        # 4th request should be denied
        allowed, retry_after = limiter.check_rate_limit(api_key="test-key-123", request_id="req-4")
    ...
    def test_rate_limiter_per_key_isolation(self):
        ...
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(api_key="key-1", request_id=f"req-1-{i}")
            assert allowed is True

        # Key 1's 3rd request is denied
        allowed, _ = limiter.check_rate_limit(api_key="key-1", request_id="req-1-3")
        assert allowed is False

        # Key 2 can still make requests
        allowed, _ = limiter.check_rate_limit(api_key="key-2", request_id="req-2-1")
        assert allowed is True
    ...
    def test_rate_limiter_ip_fallback(self):
        ...
        allowed, _ = limiter.check_rate_limit(client_ip="192.168.1.100", request_id="req-1")
        assert allowed is True

        allowed, _ = limiter.check_rate_limit(client_ip="192.168.1.100", request_id="req-2")
        assert allowed is True

        # 3rd request from same IP should be denied
        allowed, retry_after = limiter.check_rate_limit(
            client_ip="192.168.1.100", request_id="req-3"
        )
        assert allowed is False
        assert retry_after is not None
    ...
    def test_rate_limiter_prioritizes_api_key_over_ip(self):
        ...
        allowed, _ = limiter.check_rate_limit(
            api_key="test-key", client_ip="192.168.1.100", request_id="req-1"
        )
        assert allowed is True
        ...
        allowed, _ = limiter.check_rate_limit(
            api_key="test-key", client_ip="192.168.1.300", request_id="req-3"  # Yet another IP
        )
        assert allowed is False

    def test_rate_limiter_disabled_bypasses_enforcement(self):
        """Test that disabled rate limiter allows all requests."""
        limiter = RateLimiter(window_seconds=60, max_requests=2, enabled=False)  # Disabled

        # Make many requests (should all succeed)
        for i in range(10):
            allowed, retry_after = limiter.check_rate_limit(
                api_key="test-key", request_id=f"req-{i}"
            )
            assert allowed is True
            assert retry_after is None
    ...
    def test_rate_limiter_per_key_overrides(self):
        ...
        for i in range(5):
            allowed, _ = limiter.check_rate_limit(
                api_key="premium-key", request_id=f"req-premium-{i}"
            )
            assert allowed is True

        # 6th request denied
        allowed, _ = limiter.check_rate_limit(api_key="premium-key", request_id="req-premium-6")
        assert allowed is False

        # Basic key should get only 1 request
        allowed, _ = limiter.check_rate_limit(api_key="basic-key", request_id="req-basic-1")
        assert allowed is True

        # 2nd request denied
        allowed, _ = limiter.check_rate_limit(api_key="basic-key", request_id="req-basic-2")
        assert allowed is False

        # Unknown key should get default limit (2)
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(
                api_key="unknown-key", request_id=f"req-unknown-{i}"
            )
            assert allowed is True

        allowed, _ = limiter.check_rate_limit(api_key="unknown-key", request_id="req-unknown-3")
        assert allowed is False
    ...
    def test_rate_limiter_no_identifier_denies(self):
        ...
        allowed, retry_after = limiter.check_rate_limit(
            api_key=None, client_ip=None, request_id="req-1"
        )
    ...
    def test_rate_limiter_concurrent_requests(self):
        ...
        def make_request(request_id):
            allowed, _ = limiter.check_rate_limit(api_key="concurrent-key", request_id=request_id)
            with lock:
        ...
        threads = []
        for i in range(15):
            thread = threading.Thread(target=make_request, args=(f"concurrent-req-{i}",))
    ...
    def test_rate_limiter_api_key_hashing(self):
        ...
        limiter.check_rate_limit(api_key="my-secret-api-key-12345", request_id="req-1")
...
class TestRateLimiterIntegration:
    ...
    def test_burst_then_sustained_load(self):
        ...
        limiter = RateLimiter(
            window_seconds=10, max_requests=5, enabled=True  # 10 second window  # 5 requests max
        )

        # Burst: make 5 requests immediately (all should succeed)
        for i in range(5):
            allowed, _ = limiter.check_rate_limit(api_key="burst-key", request_id=f"burst-{i}")
            assert allowed is True

        # 6th immediate request should fail
        allowed, retry_after = limiter.check_rate_limit(
            api_key="burst-key", request_id="burst-fail"
        )
        assert allowed is False
        assert retry_after is not None
        ...
        allowed, _ = limiter.check_rate_limit(api_key="burst-key", request_id="post-wait")
        assert allowed is True
...
class TestRateLimiterRapidRequests:
    ...
    def test_rapid_repeated_requests_from_same_fingerprint(self):
        ...
        for i in range(5):  # More than the limit
            allowed, retry_after = limiter.check_rate_limit(
                api_key="rapid-key", request_id=f"rapid-{i}"
            )
            results.append(allowed)
    ...
    def test_fingerprint_isolation_api_key_vs_ip(self):
        ...
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(
                api_key="test-key", client_ip="192.168.1.100", request_id=f"key-{i}"
            )
            assert allowed is True

        # API key quota exhausted
        allowed, _ = limiter.check_rate_limit(
            api_key="test-key", client_ip="192.168.1.100", request_id="key-fail"
        )
        assert allowed is False

        # Same IP but no API key should have independent quota
        allowed, _ = limiter.check_rate_limit(
            api_key=None, client_ip="192.168.1.100", request_id="ip-1"
        )
        assert allowed is True
    ...
    def test_state_isolation_between_different_fingerprints(self):
        ...
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(api_key="client1-key", request_id=f"c1-{i}")
            assert allowed is True

        allowed, _ = limiter.check_rate_limit(api_key="client1-key", request_id="c1-fail")
        assert allowed is False

        # Client 2 should have full quota
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(api_key="client2-key", request_id=f"c2-{i}")
            assert allowed is True

        # Client 3 with different IP should have full quota
        for i in range(2):
            allowed, _ = limiter.check_rate_limit(client_ip="10.0.0.1", request_id=f"c3-{i}")
            assert allowed is True
...
class TestRateLimiterStateLeakage:
    ...
    def test_limiter_state_does_not_leak_between_instances(self):
        ...
        limiter1 = RateLimiter(window_seconds=60, max_requests=2, enabled=True)
        ...
        limiter2 = RateLimiter(window_seconds=60, max_requests=2, enabled=True)
    ...
    def test_bucket_cleanup_does_not_affect_other_keys(self):
        ...
        limiter = RateLimiter(window_seconds=60, max_requests=5, enabled=True)
...
class TestRateLimiterConcurrentSafety:
    ...
    def test_concurrent_access_no_race_conditions(self):
        ...
        limiter = RateLimiter(
            window_seconds=60, max_requests=20, enabled=True  # Higher limit for this test
        )
    ...
    def test_concurrent_different_keys_no_interference(self):
        ...
        def make_requests_for_key(key_name, count):
            for i in range(count):
                allowed, _ = limiter.check_rate_limit(
                    api_key=key_name, request_id=f"{key_name}-{i}"
                )
                with lock:
        ...
        for key_name in ["key1", "key2", "key3"]:
            ...
            assert (
                len([r for r in results[key_name] if not r]) == 2
            ), f"{key_name} should have 2 denied"
