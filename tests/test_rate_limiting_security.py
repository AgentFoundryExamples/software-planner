...
class TestClientIPExtraction:
    ...
    def test_default_ignores_proxy_headers(self):
        ...
        request = Mock()
        request.headers = {"X-Forwarded-For": "1.2.3.4, 5.6.7.8", "X-Real-IP": "9.10.11.12"}
        request.client = Mock()
...
class TestEmptyAPIKeyHandling:
    ...
    def test_whitespace_api_key_uses_ip_limiting(self, client_with_rate_limiter):
        ...
        response = client.post(
            "/api/v1/plan", json={"description": "Test 3"}, headers={"X-API-Key": "   "}
        )
        assert response.status_code == 429
            ...
            try:
                for i in range(5):
                    allowed, _ = limiter.check_rate_limit(
                        api_key=f"thread-{thread_id}-key", request_id=f"req-{thread_id}-{i}"
                    )
                    with lock:
